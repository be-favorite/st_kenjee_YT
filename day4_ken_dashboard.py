# -*- coding: utf-8 -*-
# source: https://www.youtube.com/watch?v=Yk-unX4KnV4
...
def load_data():
    # Reading data
    df_agg = pd.read_csv("./ken_jee_youtube/Aggregated_Metrics_By_Video.csv").iloc[1:, :] # skip a first row
    df_agg_sub = pd.read_csv("./ken_jee_youtube/Aggregated_Metrics_By_Country_And_Subscriber_Status.csv")
    df_comments = pd.read_csv("./ken_jee_youtube/Aggregated_Metrics_By_Video.csv")
    df_time = pd.read_csv("./ken_jee_youtube/Video_Performance_Over_Time.csv")
    ...
    # return clean_names(df_agg), clean_names(df_agg_sub), clean_names(df_comments), clean_names(df_time)
...
#     >> filter(_.video_publish_time >= metric_date_12mo) 
#     >> _.median()
# )

#create differences from the median for values 
#Just numeric columns 
numeric_cols = np.array((df_agg_diff.dtypes == 'float64') | (df_agg_diff.dtypes == 'int64')) # get numeric columns
df_agg_diff.iloc[:,numeric_cols] = (df_agg_diff.iloc[:,numeric_cols] - median_agg).div(median_agg) # compute (value - med)/med
# -> to get percent differences
...
views_days.columns = ['days_published','mean_views','median_views','80pct_views','20pct_views'] # rename
views_days = views_days[views_days['days_published'].between(0,30)] # 0-30일까지만
...
if add_sidebar == 'Aggregate Metrics':
    ...
    for i in metric_medians6mo.index:
        with columns[count]:
            # last 6months와 last 12monts 메디안 대비 변화 계산
            delta = (metric_medians6mo[i] - metric_medians12mo[i])/metric_medians12mo[i]
            st.metric(label = i, value = round(metric_medians6mo[i], 1), delta = "{:.2%}".format(delta))
            count += 1
            if count >= 5: # 5개 지표를 하나의 행으로. 2번째 row에 기록을 위해 count 0으로 초기화.
                count = 0
